...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('backlog_age_view').select('*').eq('team_id', filters.teamId)
    if (filters.siteId) query = query.eq('site_id', filters.siteId)
    if (filters.severity) query = query.eq('impact', filters.severity)
    query = query.order('days_old', { ascending: false }).limit(50)

    const { data, error } = await query
    if (error) {
      return NextResponse.json({ error: 'Failed to fetch backlog age', code: 'DATABASE_ERROR', details: error.message }, { status: 500 })
    }
