...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('coverage_view').select('*').eq('team_id', filters.teamId)
    if (filters.siteId) query = query.eq('site_id', filters.siteId)
    query = query.order('last_scan_date', { ascending: false, nullsFirst: false })

    const { data, error } = await query
    if (error) {
      return NextResponse.json({ error: 'Failed to fetch coverage', code: 'DATABASE_ERROR', details: error.message }, { status: 500 })
    }
