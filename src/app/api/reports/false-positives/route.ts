...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('false_positive_view').select('*').eq('team_id', filters.teamId)
    if (filters.siteId) query = query.eq('site_id', filters.siteId)

    const { data, error } = await query
    if (error) {
      return NextResponse.json({ error: 'Failed to fetch false positives', code: 'DATABASE_ERROR', details: error.message }, { status: 500 })
    }

    return NextResponse.json({ success: true, data: data || [], timestamp: new Date().toISOString() })
  } catch (error: any) {
    console.error('[false-positives] Error:', error)
