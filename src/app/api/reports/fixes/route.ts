...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('fix_throughput_view').select('*').eq('team_id', filters.teamId)
    ...
    if (error) {
      return NextResponse.json({ error: 'Failed to fetch fix throughput', code: 'DATABASE_ERROR', details: error.message }, { status: 500 })
    }
