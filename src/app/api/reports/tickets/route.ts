...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('tickets_view').select('*').eq('team_id', filters.teamId)
    if (filters.siteId) query = query.eq('site_id', filters.siteId)

    const { data, error } = await query
    if (error) {
      return NextResponse.json({ error: 'Failed to fetch tickets data', code: 'DATABASE_ERROR', details: error.message }, { status: 500 })
    }
